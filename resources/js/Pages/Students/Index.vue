...
const props = defineProps({
    students: {
        type: Array as PropType<App.Models.Student[]>,
        required: true
    },
    users: {
...
const selectedstudent = ref<App.Models.Teacher>();
...
const submit = () => {
    form.post(route("students.store"), {
        onSuccess: () => {
            toast.add({ severity: 'success', summary: 'Message de réussite', detail: 'le étudiante a été créé avec succès', life: 3000 });
            resetForm();
            closeDialog();
        },
        onError: () => {
...
const update = (id) => {
    form.put(route("students.update", { student: id }), {
        onSuccess: () => {
            toast.add({ severity: 'success', summary: 'Message de réussite', detail: 'Le étudiante a été bien modifier', life: 3000 });
            resetForm();
            closeDialogEdit();
        },
        onError: () => {
    ...
    confirm.require({
        ...
        accept: () => {
            ...
            form.delete(route("students.destroy", { student: id }), {
                onSuccess: () => {
                    toast.add({ severity: 'info', summary: 'Message de réussite', detail: 'Le étudiante a été bien supprimer', life: 3000 });
                    resetForm();
                    closeDialogEdit();
                },
                onError: () => {
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">List des etudiantes</h2>
        </template>
        <Toolbar>
            <template #start>
        ...
        <DataTable showGridlines :value="students" responsiveLayout="scroll" :paginator="true" :rows="4">
            <template #header>
                <div class="table-header flex flex-column md:flex-row md:justiify-content-between">
                    <h5 class="mb-2 md:m-0 p-as-md-center">Gérer les etudiantes</h5>
        ...
        <!-- Create dialog -->
        <Dialog header="Nouveau etudiantes" v-model:visible="displayDialog"
        ...
        <!-- Edit dialog -->
        <Dialog :header="'Modifier le etudiantes ' + form.first_name + ' ' + form.last_name"
            ...
            <template #footer>
                <Button label="Soumettre" icon="pi pi-check" class="p-button-success" autofocus
                    @click="update(selectedstudent?.id)" :loading="form.processing" />
            </template>
        </Dialog>
    </AuthenticatedLayout>
